...
// Settings
var isMultifactActive = false;
var isChainTriviaActive = false;
var isEshrugActive = false;
var isSpamActive = false;
...
async function onMessageHandler(target, context, msg, self) {
  ...
  if (context['display-name'] == "FeelsStrongBot" && msg == "trivia ended nam") {
    if (isChainTriviaActive)
      setTimeout(doTrivia, randTime(timeSVariable) + timeSConstant);
    return;
  }
...
function farm(identity) {
  ...
  let stdActions = ["+ed", "+eg", "$fish trap reset", "Okayeg gib eg", "?cookie", "¿taco pepeSenora", "%hw"].sort(() => Math.random() - 0.5);
  let timer = 0;
  for (let action of stdActions) {
  ...
  for (let action of potatoActions) {
    setTimeout(function () { say("forsen", action, identity); }, timer);
...
async function main() {
  ...
  let client1 = new tmi.client({ connection: { reconnect: true, secure: true }, identity: { username: identities[0].username, password: identities[0].password }, channels: ['forsen'] });
  ...
  let client2 = new tmi.client({ connection: { reconnect: true, secure: true }, identity: { username: identities[1].username, password: identities[1].password }, channels: ['forsen'] });
  client2.connect();
  identities[1].client = client2;

  let client3 = new tmi.client({ connection: { reconnect: true, secure: true }, identity: { username: identities[2].username, password: identities[2].password }, channels: ['forsen'] });
  client3.connect();
  ...
  app.post('/command', async (req, res) => {
    ...
    let output = await processCommand(message);
